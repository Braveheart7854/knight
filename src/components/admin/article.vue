<template>
  <div class="main-wrapper">
    <md-table-card>
      <md-toolbar>
        <md-button class="md-icon-button">
          <md-icon>filter_list</md-icon>
        </md-button>

        <md-button class="md-icon-button">
          <md-icon>search</md-icon>
        </md-button>
      </md-toolbar>

      <md-table md-sort="dessert" md-sort-type="desc" @select="onSelect" @sort="onSort">
        <md-table-header>
          <md-table-row>
            <md-table-head md-sort-by="dessert">id</md-table-head>
            <md-table-head md-sort-by="calories"><span>权限</span></md-table-head>
            <md-table-head md-sort-by="fat" ><span>分类</span></md-table-head>
            <md-table-head>
              <md-icon>message</md-icon>
              <span>标题</span>
            </md-table-head>
            <md-table-head md-sort-by="fat" ><span>时间</span></md-table-head>
          </md-table-row>
        </md-table-header>

        <md-table-body>
          <md-table-row v-for="(row, rowIndex) in nutrition" :key="rowIndex" :md-item="row" md-auto-select md-selection>
            <md-table-cell> {{row.id}} </md-table-cell>
            <md-table-cell> {{row.category }}</md-table-cell>
            <md-table-cell> {{row.permission}}</md-table-cell>
            <md-table-cell> {{row.title}} </md-table-cell>
            <md-table-cell> {{row.created}} </md-table-cell>
          </md-table-row>
        </md-table-body>
      </md-table>

      <md-table-pagination
        md-size="5"
        md-total="10"
        md-page="1"
        md-label="Rows"
        md-separator="of"
        :md-page-options="[5, 10, 25, 50]"
        @pagination="onPagination"></md-table-pagination>
    </md-table-card>
  </div>
</template>
<script>
  export default {
    data: () => ({
      nutrition: [
        {
          id: '1',
          category: 'ice_cream',
          permission: '自言自语',
          title: 'some title',
          created: '2017-06-05'
        },
        {
          id: '2',
          category: 'ice_cream',
          permission: '159',
          title: '6.0',
          created: '2017-06-05'
        },
      ]
    }),
    methods: {
      onPagination() {

      },
      onSelect() {

      },
      onSort() {

      }
    },
  }
</script>